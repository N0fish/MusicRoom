### User-service :
Сервис отвечает за визитку пользователя, его настройки, публичность и социальные связи.

Этот сервис получает X-User-Id от API Gateway (который проверяет JWT).
Все защищённые эндпоинты доступны только через API Gateway.

### Основные функции:
🔹 Профиль пользователя:
- displayName
- username (уникальный читаемый ник, генерируется автоматически: sleepy-strawberry, angry-coffee…)
- avatarUrl (стандартный SVG или сгенерированный пользователем)
- публичное/друзьям/приватное отображение
- bio (public / friends / private)
- предпочтения: жанры, артисты, настроение (массивы строк)

🔹 Приватность
- public — видно всем
- friends — видно только друзьям
- private — отображается минимальная информация (аватар по умолчанию)

🔹 Система друзей:
- отправка заявки
- принятие заявки
- отклонение
- удаление друга
- авто-принятие встречной заявки

🔹 Аватар:
- стандартный дефолтный
- генерация SVG (идентикон) через /users/me/avatar/random

🔹 Поиск других пользователей:
- поиск по username
- поиск по displayName

## Эндпоинты (только через API Gateway)

`GET /user/me`
Возвращает полный профиль, включая все поля, приватные данные и предпочтения.
Ex :
```json
{
  "id": "profile-uuid",
  "userId": "auth-user-id",
  "username": "sleepy-strawberry",
  "displayName": "Alla",
  "avatarUrl": "data:image/svg+xml;utf8,...",
  "hasCustomAvatar": true,
  "publicBio": "Hi!",
  "friendsBio": "My close friends know...",
  "privateBio": "Very private info",
  "visibility": "friends",
  "preferences": {
    "genres": ["rock", "pop"],
    "artists": ["Muse"],
    "moods": ["energetic"]
  },
  "createdAt": "...",
  "updatedAt": "..."
}
```
Ошибки:
- 401 — нет X-User-Id → API Gateway не пропустил токен.


`PATCH /users/me`
изменить свой профиль
Можно изменить:
- displayName
- publicBio
- friendsBio
- privateBio
- visibility (public | friends | private)
- preferences (жанры, артисты, настроение)

Нельзя изменять напрямую:
- avatarUrl → только через /users/me/avatar/random
- username → генерируется автоматически

ex :
```json
{
	"displayName": "lofafa",
	"publicBio": "DJ from Russia",
  "friendsBio": "Только для моих",
  "privateBio": "For mee",
	"visibility": "friends",
	"preferences": {
    "genres": ["techno", "house"],
    "artists": ["Syuzi Dogs"],
    "moods": ["party"]
  }
}
```
Ошибки:
- 400 — невалидные поля
- 401 — без авторизации
- 500 — внутренняя ошибка


`GET /users/{id}`
публичный профиль другого пользователя в зависимости от режима приватности и дружбы.
ex responce :
```json
{
	"userId": "target-user-id",
	"username": "wild-lemur",
	"displayName": "",
	"avatarUrl": "/static/avatars/default.svg",
	"visibility": "friends",
	"preferences": {}
}
```
Ошибки:
- 400 — неверный ID
- 404 — профиль отсутствует


`GET /users/search?query=lo`
Ищет по:
- username (точное/частичное)
- displayName
```json
{
	"items": [
		{
			"userId": "32dee96c-6da1-4671-b4a3-1a126169eb0f",
			"username": "noisy-comet",
			"displayName": "lofafa",
			"avatarUrl": "/static/avatars/default.svg"
		},
		{
			"userId": "5234712b-3db3-4778-987a-e66ca5c628f1",
			"username": "shiny-dragon",
			"displayName": "lolo",
			"avatarUrl": "/static/avatars/default.svg"
		}
	]
}
```
Ошибки:
- 400 — нет query / слишком длинный
- 401 — авторизация отсутствует


## Друзья
`GET /users/me/friends`
Список друзей текущего пользователя:
ex :
```json
{
	"items": null
}
```
```json
{
  {
    "userId": "5234712b-3db3-4778-987a-e66ca5c628f1",
    "username": "shiny-dragon",
    "displayName": "lolo",
    "avatarUrl": "/static/avatars/default.svg"
  }
}
```

`POST /users/me/friends/{id}/request`
отправить заявку в друзья
Ошибки:
- 400 — id некорректный / сам себе / уже друзья
- 404 — пользователь не найден
- 500 — внутренний сбой

`POST /users/me/friends/{id}/accept`
принять заявку
Ошибки:
- 404 — нет входящей заявки
- 400 — некорректный ID

`POST /users/me/friends/{id}/reject`
отклонить заявку
Ошибки:
- 404 — нет входящей заявки
- 400 — неправильный ID

`DELETE /users/me/friends/{id}`
удалить из друзей
Ошибки:
- 400 — неправильный ID
- 500 — ошибка базы


## Аватар
`POST /users/me/avatar/random`
сгенерировать новый SVG-аватар
Создаёт SVG identicon
ex :
```json
{
  "id": "...",
  "userId": "...",
  "avatarUrl": "data:image/svg+xml;utf8,...",
  "hasCustomAvatar": true
}
```

## Как работает username
Генерируется автоматически при первой загрузке профиля:
```bash
"adjective-noun"
sleepy-strawberry
angry-coffee
giant-goose
```

Генератор:
- выбирает случайную пару
- проверяет уникальность в БД
- повторяет до 20 раз
- Если все занято → ошибка в логах (маловероятно).

## Структура данных
user_profiles
- id
- user_id (UUID из auth-service)
- display_name
- username
- avatar_url
- visibility
- public_bio / friends_bio / private_bio
- preferences (JSONB)
friend_requests
- from_user_id
- to_user_id
- status: pending / accepted / rejected
user_friends
- user1_id
- user2_id
